...
describe('Wallet service', function() {
  ...
  describe('#getUtxos', function() {
    ...
    it('should get UTXOs for wallet addresses', function(done) {
      helpers.stubUtxos(server, wallet, [1, 2], function() {
        server.getUtxos({}, function(err, utxos) {
          ...
          server.getMainAddresses({}, function(err, addresses) {
            var utxo = utxos[0];
            var address = lodash.find(addresses, {
    ...
    it('should get UTXOs for specific addresses', function(done) {
      helpers.stubUtxos(server, wallet, [1, 2, 3], function(utxos) {
        ...
        var amount = lodash.sumBy(lodash.filter(utxos, {
          address: address
        }), atomicsAccessor);
        server.getUtxos({
          addresses: [address]
  ...
  describe('Multiple request Pub Keys', function() {
    ...
    var getAuthServer = function(serviceName, copayerId, privKey, cb) {
      var msg = 'dummy';
      var sig = helpers.signMessage(serviceName, msg, privKey);
      Server.getInstanceWithAuth({
    ...
    beforeEach(function(done) {
      ...
      var sig = helpers.signRequestPubKey(serviceName, requestPubKeyStr, xPrivKey);
    ...
    it('should trigger notification when balance of non-prioritary addresses is updated', function(done) {
      ...
      async.series([
        ...
        function(next) {
          server.getNotifications({}, function(err, notifications) {
            should.not.exist(err);
            var last = lodash.last(notifications);
  ...
  describe('#getFeeLevels', function() {
    ...
    it('should get default fees if network cannot be accessed', function(done) {
      ...
      server.getFeeLevels({}, function(err, fees) {
        ...
        var defaults = lodash.fromPairs(lodash.map(Defaults.FEE_LEVELS, function(item) {
  ...
  describe('#createTx', function() {
    describe('Tx proposal creation & publishing', function() {
          ...
          helpers.stubUtxos(server, wallet, [1, 3, 2], function(utxos) {
            server.getUtxos({}, function(err, utxos) {
              ...
              server.createTx(txOpts, function(err, tx) {
                ...
                var txids = lodash.map(tx.inputs, 'txid');
        ...
        it('should return already published tx if same foreign ID is specified and tx already published', function(done) {
          helpers.stubUtxos(server, wallet, [2, 2, 2], function() {
            ...
            server.createTx(txOpts, function(err, tx) {
              ...
              should.exist(tx);
              tx.id.should.equal('123');
              var publishOpts = helpers.getProposalSignatureOpts(serviceName, tx, TestData.copayers[0].privKey_1H_0);
        ...
        it('should be able to publish a temporary tx proposal', function(done) {
          helpers.stubUtxos(server, wallet, [1, 2], function() {
            ...
            server.createTx(txOpts, function(err, txp) {
              should.not.exist(err);
              should.exist(txp);
              var publishOpts = helpers.getProposalSignatureOpts(serviceName, txp, TestData.copayers[0].privKey_1H_0);
              ...
              server.getNotifications({}, function(err, notifications) {
                ...
                server.publishTx(publishOpts, function(err) {
                  ...
                  server.getNotifications({}, function(err, notifications) {
                    ...
                    var n = lodash.find(notifications, {
                      'type': 'NewTxProposal'
                    });
                    should.exist(n);
        ...
        it('should fail to publish a temporary tx proposal if utxos are locked by other pending proposals', function(done) {
          ...
          async.waterfall([
            ...
            function(txp, next) {
              ...
              should.exist(txp1);
              should.exist(txp2);
              var publishOpts = helpers.getProposalSignatureOpts(serviceName, txp1, TestData.copayers[0].privKey_1H_0);
              server.publishTx(publishOpts, next);
            },
            function(txp, next) {
              var publishOpts = helpers.getProposalSignatureOpts(serviceName, txp2, TestData.copayers[0].privKey_1H_0);
              server.publishTx(publishOpts, function(err) {
                should.exist(err);
            ...
            function(txp3, next) {
              should.exist(txp3);
              var publishOpts = helpers.getProposalSignatureOpts(serviceName, txp3, TestData.copayers[0].privKey_1H_0);
              server.publishTx(publishOpts, next);
            },
            function(txp, next) {
              server.getPendingTxs({}, function(err, txs) {
      ...
      it('should fail gracefully when exception thrown on raw tx creation', function(done) {
        helpers.stubUtxos(server, wallet, 1, function() {
          var txpStub = sinon.stub(TxProposal.prototype, '_buildTx');
          txpStub.throws({
            name: 'dummy',
            ...
            server.createTx(txOpts, function(err, tx) {
              ...
              server.getPendingTxs({}, function(err, txs) {
                ...
                server.getBalance({}, function(err, balance) {
                  should.not.exist(err);
                  balance.totalAmount.should.equal(3.6e8);
                  var amountInputs = lodash.sumBy(txs[0].inputs, atomicsAccessor);
      ...
      it('should shuffle outputs unless specified', function(done) {
        helpers.stubUtxos(server, wallet, 1, function() {
          ...
          server.createTx(txOpts, function(err, txp) {
            ...
            should.exist(txp);
            var t = txp.getTx();
            var changeOutput = t.getChangeOutput()[atomicsAccessor];
            var outputs = lodash.without(lodash.map(t.outputs, atomicsAccessor), changeOutput);
      ...
      it('should return inputs in random order', function(done) {
        ...
        helpers.stubUtxos(server, wallet, lodash.range(1, 31), function(utxos) {
          ...
          server.createTx(txOpts, function(err, txp) {
            should.not.exist(err);
            should.exist(txp);
            var amounts = lodash.map(txp.inputs, atomicsAccessor);
      ...
      it('should correct fee if resulting change would be below threshold', function(done) {
        helpers.stubUtxos(server, wallet, ['200bit', '500satoshi'], function() {
          ...
          server.createTx(txOpts, function(err, txp) {
            ...
            var changeOutput = txp.getTx().getChangeOutput();
    ...
    it('should return inputs in random order', function(done) {
      ...
      helpers.stubUtxos(server, wallet, lodash.range(1, 31), function(utxos) {
        server.getSendMaxInfo({
          ...
          should.not.exist(err);
          should.exist(info);
          var amounts = lodash.map(info.inputs, atomicsAccessor);
          amounts.length.should.equal(30);
          lodash.every(amounts, function(amount, i) {
            if (i == 0) return true;
            return amount < amounts[i - 1];
    ...
    describe('Multisig', function() {
      ...
      it('should fail on wrong number of invalid signatures', function(done) {
        server.getPendingTxs({}, function(err, txs) {
          ...
          var signatures = lodash.take(helpers.clientSign(tx, TestData.copayers[0].xPrivKey_44H_0H_0H), tx.inputs.length - 1);
          server.signTx({
            txProposalId: txid,
            signatures: signatures,
          }, function(err) {
  ...
  describe('#getTxs', function() {
    ...
    it('should pull 4 txs, down to to time 60', function(done) {
      server.getTxs({
        ...
        should.not.exist(err);
        var times = lodash.map(txps, 'createdOn');
  ...
  describe('#getNotifications', function() {
    ...
    it('should pull all notifications', function(done) {
      server.getNotifications({}, function(err, notifications) {
        should.not.exist(err);
        var types = lodash.map(notifications, 'type');
        types.should.deep.equal(['NewCopayer', 'NewAddress', 'NewAddress', 'NewTxProposal', 'NewTxProposal', 'NewTxProposal']);
        var walletIds = lodash.uniq(lodash.map(notifications, 'walletId'));
        walletIds.length.should.equal(1);
        walletIds[0].should.equal(wallet.id);
        var creators = lodash.uniq(lodash.compact(lodash.map(notifications, 'creatorId')));
    ...
    it('should pull notifications after a given notification id', function(done) {
      server.getNotifications({}, function(err, notifications) {
        should.not.exist(err);
        var from = lodash.head(lodash.takeRight(notifications, 2)).id; // second to last
        server.getNotifications({
          notificationId: from,
          minTs: +Date.now() - (60 * 1000),
        }, function(err, res) {
    ...
    it('should return empty if no notifications found after a given id', function(done) {
      server.getNotifications({}, function(err, notifications) {
        should.not.exist(err);
        var from = lodash.head(lodash.takeRight(notifications)).id; // last one
        server.getNotifications({
          notificationId: from,
        }, function(err, res) {
          should.not.exist(err);
  ...
  describe('#getTxHistory', function() {
    ...
    it('should get various paginated tx history', function(done) {
      ...
      var txs = lodash.map(timestamps, function(ts, idx) {
        return {
          txid: (idx + 1).toString(),
          confirmations: ts / 10,
          fees: 100,
    ...
    it('should handle invalid tx in  history ', function(done) {
      var h = lodash.clone(TestData.history);
      h.push({
        txid: 'xx'
      })
  ...
  describe('#getTxHistory cache', function() {
    ...
    it('should store partial cache tx history from explorer', function(done) {
      ...
      var storeTxHistoryCacheSpy = sinon.spy(server.getStorage(), 'storeTxHistoryCache');
    ...
    it('should get cached # of confirmations if current height unknown', function(done) {
      ...
      helpers.stubHistory(serviceName, h);
      var storeTxHistoryCacheSpy = sinon.spy(Storage.prototype, 'storeTxHistoryCache');
    ...
    describe('Downloading history', function() {
      ...
      it('from 0 to 200, two times, random', function(done) {
        var indexes = lodash.range(0, 200, 5);
        ...
        server.scan({}, function(err) {
          ...
          server.getWallet({}, function(err, wallet) {
            ...
            server.getStorage().fetchAddresses(wallet.id, function(err, addresses) {
              ...
              var paths = lodash.map(addresses, 'path');
