...
describe('Wallet service', function () {
    ...
    describe('#createAddress', function () {
        ...
        describe('shared wallets (BIP45)', function () {
            ...
            it('should create many addresses on simultaneous requests', function (done) {
                ...
                async.mapSeries(lodash.range(N), function (i, cb) {
                    ...
                    const x = lodash.map(addresses, 'path');
        ...
        describe('1-of-1 (BIP44 & P2PKH)', function () {
            ...
            it('should fail to create more consecutive addresses with no activity than allowed', function (done) {
                const MAX_MAIN_ADDRESS_GAP_old = Defaults.MAX_MAIN_ADDRESS_GAP;
    ...
    describe('Preferences', function () {
        ...
        it('should validate entries', function (done) {
            const invalid = [{
                ...
                preferences: {
                    email: `dummy@${  lodash.repeat('domain', 50)}`,
                },
                expected: 'email'
            }, {
                preferences: {
    ...
    describe('#getUtxos', function () {
        ...
        it('should get UTXOs for wallet addresses', function (done) {
            helpers.stubUtxos(server, wallet, [1, 2], function () {
                server.getUtxos({}, function (err, utxos) {
                    ...
                    lodash.sumBy(utxos, atomicsAccessor).should.equal(3 * 1e8);
                    server.getMainAddresses({}, function (err, addresses) {
                        const utxo = utxos[0];
                        const address = lodash.find(addresses, {
        ...
        it('should get UTXOs for specific addresses', function (done) {
            helpers.stubUtxos(server, wallet, [1, 2, 3], function (utxos) {
                lodash.uniqBy(utxos, 'address').length.should.be.above(1);
                const address = utxos[0].address;
                const amount = lodash.sumBy(lodash.filter(utxos, {
                    address: address
                }), atomicsAccessor);
                server.getUtxos({
                    addresses: [address]
                    ...
                    should.not.exist(err);
                    should.exist(utxos);
                    lodash.sumBy(utxos, atomicsAccessor).should.equal(amount);
                    done();
                });
            });
        });
    ...
    describe('Multiple request Pub Keys', function () {
        let server; let wallet;
        let opts; let reqPrivKey; let ws;
        const getAuthServer = function (serviceName, copayerId, privKey, cb) {
            const msg = 'dummy';
            const sig = helpers.signMessage(serviceName, msg, privKey);
            Server.getInstanceWithAuth({
        ...
        beforeEach(function (done) {
            ...
            const sig = helpers.signRequestPubKey(serviceName, requestPubKeyStr, xPrivKey);

            const copayerId = Copayer._xPubToCopayerId(TestData.copayers[0].xPubKey_44H_0H_0H);
            opts = {
                copayerId: copayerId,
            ...
            it('should be able to re-gain access from xPrivKey', function (done) {
                ws.addAccess(opts, function (err, res) {
                    ...
                    server.getBalance(res.wallet.walletId, function (err, bal) {
                        ...
                        getAuthServer(serviceName, opts.copayerId, reqPrivKey, function (err, server2) {
            ...
            it('should fail to access with wrong privkey after gaining access', function (done) {
                ws.addAccess(opts, function (err, res) {
                    ...
                    server.getBalance(res.wallet.walletId, function (err, bal) {
                        ...
                        (getAuthServer(serviceName, opts.copayerId, privKey, function (err, server2) {
                            err.code.should.equal('NOT_AUTHORIZED');
                            done();
                        }));
                    });
        ...
        it('should get balance', function (done) {
            helpers.stubUtxos(server, wallet, [1, 'u2', 3], function () {
                server.getBalance({}, function (err, balance) {
                    ...
                    balance.totalAmount.should.equal(helpers.toAtomic(serviceName, 6));
                    balance.lockedAmount.should.equal(0);
                    balance.availableAmount.should.equal(helpers.toAtomic(serviceName, 6));

                    balance.totalConfirmedAmount.should.equal(helpers.toAtomic(serviceName, 4));
                    balance.lockedConfirmedAmount.should.equal(0);
                    balance.availableConfirmedAmount.should.equal(helpers.toAtomic(serviceName, 4));

                    should.exist(balance.byAddress);
                    balance.byAddress.length.should.equal(2);
                    balance.byAddress[0].amount.should.equal(helpers.toAtomic(serviceName, 4));
            ...
            helpers.stubUtxos(server, wallet, 1, function (utxos) {
                server.createAddress({}, function (err, address) {
                    ...
                    server.getBalance({}, function (err, balance) {
                        ...
                        balance.byAddress[0].amount.should.equal(helpers.toAtomic(serviceName, 1));
    ...
    describe('#getBalance 2 steps', function () {
        let server; let wallet; let clock;
        const _threshold = Defaults.TWO_STEP_BALANCE_THRESHOLD;
        beforeEach(function (done) {
            ...
            async.series([
                ...
                function (next) {
                    server.getBalance({
                        ...
                        should.not.exist(err);
                        should.exist(balance);
                        balance.totalAmount.should.equal(helpers.toAtomic(serviceName, 3));
                        next();
                    });
                },
                function (next) {
                ...
                function (next) {
                    server.getNotifications({}, function (err, notifications) {
                        should.not.exist(err);
                        const last = lodash.last(notifications);
                        last.type.should.equal('BalanceUpdated');
                        const balance = last.data;
                        balance.totalAmount.should.equal(helpers.toAtomic(serviceName, 3.5));
                        next();
                    });
                },
            ], function (err) {
    ...
    describe('#getFeeLevels', function () {
        let server; let wallet; let levels;
        before(function () {
        ...
        it('should get current fee levels', function (done) {
            helpers.stubFeeLevels(serviceName, {
                1: 40000,
                2: 20000,
                6: 18000,
                24: 9000,
            ...
            server.getFeeLevels({}, function (err, fees) {
                should.not.exist(err);
                fees = lodash.fromPairs(lodash.map(fees, function (item) {
        ...
        it('should get default fees if network cannot be accessed', function (done) {
            ...
            server.getFeeLevels({}, function (err, fees) {
                ...
                const defaults = lodash.fromPairs(lodash.map(Defaults.FEE_LEVELS, function (item) {
        ...
        it('should get monotonically decreasing fee values', function (done) {
            lodash.find(Defaults.FEE_LEVELS, {
                nbBlocks: 6
        ...
        it('should fail to create address when wallet is not complete', function (done) {
            new Server({
                ...
                server.createWallet(walletOpts, function (err, walletId) {
                    ...
                    server.joinWallet(copayerOpts, function (err, result) {
                        should.not.exist(err);
                        helpers.getAuthServer(serviceName, result.copayerId, function (server) {
                            server.createAddress({}, function (err, address) {
                                should.not.exist(address);
                                should.exist(err);
        ...
        it('should fail to create tx when wallet is not complete', function (done) {
            new Server({
                ...
                server.createWallet(walletOpts, function (err, walletId) {
                    ...
                    server.joinWallet(copayerOpts, function (err, result) {
                        should.not.exist(err);
                        helpers.getAuthServer(serviceName, result.copayerId, function (server, wallet) {
                            const txOpts = {
                                outputs: [{
                                    toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                                    amount: 0.8e8
    ...
    describe('#createTx', function () {
        describe('Tx proposal creation & publishing', function () {
            ...
            it('should create a tx', function (done) {
                helpers.stubUtxos(server, wallet, [1, 2], function () {
                    ...
                    server.createTx(txOpts, function (err, tx) {
                        ...
                        tx.amount.should.equal(helpers.toAtomic(serviceName, 0.8));
                    ...
                    helpers.stubUtxos(server, wallet, [1, 3, 2], function (utxos) {
                        server.getUtxos({}, function (err, utxos) {
                            ...
                            server.createTx(txOpts, function (err, tx) {
                                ...
                                const txids = lodash.map(tx.inputs, 'txid');
                ...
                it('should be able to specify inputs & absolute fee', function (done) {
                    helpers.stubUtxos(server, wallet, [1, 2], function (utxos) {
                        ...
                        server.createTx(txOpts, function (err, tx) {
                            ...
                            t.getChangeOutput()[atomicsAccessor].should.equal(3e8 - 0.8e8 - 1000e2);
                            done();
                        });
                    });
                });
                    ...
                    helpers.stubUtxos(server, wallet, 2, function () {
                        ...
                        server.createTx(txOpts, function (err, tx) {
                            ...
                            server.createTx(txOpts, function (err, tx) {
                                ...
                                should.exist(tx);
                                tx.id.should.equal('123');
                                server.getStorage().fetchTxs(wallet.id, {}, function (err, txs) {
                                    should.not.exist(err);
                                    should.exist(txs);
                ...
                it('should return already published tx if same foreign ID is specified and tx already published', function (done) {
                    helpers.stubUtxos(server, wallet, [2, 2, 2], function () {
                        ...
                        server.createTx(txOpts, function (err, tx) {
                            ...
                            should.exist(tx);
                            tx.id.should.equal('123');
                            const publishOpts = helpers.getProposalSignatureOpts(serviceName, tx, TestData.copayers[0].privKey_1H_0);
                ...
                it('should be able to publish a temporary tx proposal', function (done) {
                    helpers.stubUtxos(server, wallet, [1, 2], function () {
                        ...
                        server.createTx(txOpts, function (err, txp) {
                            should.not.exist(err);
                            should.exist(txp);
                            const publishOpts = helpers.getProposalSignatureOpts(serviceName, txp, TestData.copayers[0].privKey_1H_0);
                ...
                it('should delay NewTxProposal notification until published', function (done) {
                    helpers.stubUtxos(server, wallet, [1, 2], function () {
                        ...
                        server.createTx(txOpts, function (err, txp) {
                            ...
                            server.getNotifications({}, function (err, notifications) {
                                should.not.exist(err);
                                lodash.map(notifications, 'type').should.not.contain('NewTxProposal');
                                const publishOpts = helpers.getProposalSignatureOpts(serviceName, txp, TestData.copayers[0].privKey_1H_0);
                                server.publishTx(publishOpts, function (err) {
                                    should.not.exist(err);
                                    server.getNotifications({}, function (err, notifications) {
                                        ...
                                        const n = lodash.find(notifications, {
                                            type: 'NewTxProposal'
                                        });
                                        should.exist(n);
                ...
                it('should fail to publish tx proposal not signed by the creator', function (done) {
                    helpers.stubUtxos(server, wallet, [1, 2], function () {
                        ...
                        server.createTx(txOpts, function (err, txp) {
                            ...
                            const publishOpts = {
                                txProposalId: txp.id,
                                proposalSignature: helpers.signMessage(serviceName, txp.getRawTx(), TestData.copayers[1].privKey_1H_0),
                            };
                ...
                it('should fail to publish a temporary tx proposal if utxos are locked by other pending proposals', function (done) {
                    ...
                    async.waterfall([
                        ...
                        function (txp, next) {
                            ...
                            should.exist(txp1);
                            should.exist(txp2);
                            const publishOpts = helpers.getProposalSignatureOpts(serviceName, txp1, TestData.copayers[0].privKey_1H_0);
                            server.publishTx(publishOpts, next);
                        },
                        function (txp, next) {
                            const publishOpts = helpers.getProposalSignatureOpts(serviceName, txp2, TestData.copayers[0].privKey_1H_0);
                            server.publishTx(publishOpts, function (err) {
                                should.exist(err);
                        ...
                        function (txp3, next) {
                            should.exist(txp3);
                            const publishOpts = helpers.getProposalSignatureOpts(serviceName, txp3, TestData.copayers[0].privKey_1H_0);
                            server.publishTx(publishOpts, next);
                        },
                        function (txp, next) {
                            server.getPendingTxs({}, function (err, txs) {
            ...
            it('should support creating a tx with no change address', function (done) {
                helpers.stubUtxos(server, wallet, [1, 2], function () {
                    const max = 3e8 - 7000; // Fees for this tx at 100bits/kB = 7000 sat
                    const txOpts = {
                        outputs: [{
                            toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                            amount: max,
                    ...
                    server.createTx(txOpts, function (err, txp) {
                        should.not.exist(err);
                        should.exist(txp);
                        const t = txp.getTx().toObject();
                        t.outputs.length.should.equal(1);
                        t.outputs[0][atomicsAccessor].should.equal(max);
                        done();
                    });
                });
            });
            ...
            it('should fail gracefully when exception thrown on raw tx creation', function (done) {
                helpers.stubUtxos(server, wallet, 1, function () {
                    const txpStub = sinon.stub(TxProposal.prototype, '_buildTx');
                    txpStub.throws({
                        name: 'dummy',
            ...
            it('should fail to create a tx exceeding max size in kb', function (done) {
                const _oldDefault = Defaults.MAX_TX_SIZE_IN_KB;
                Defaults.MAX_TX_SIZE_IN_KB = 1;
                helpers.stubUtxos(server, wallet, lodash.range(1, 10, 0), function () {
                    const txOpts = {
                        outputs: [{
                            toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                            amount: 8e8,
            ...
            it('should create tx with 0 change output', function (done) {
                helpers.stubUtxos(server, wallet, 1, function () {
                    const fee = 4100; // The exact fee of the resulting tx
                    const amount = 1e8 - fee;
                    ...
                    server.createTx(txOpts, function (err, tx) {
                        ...
                        should.exist(tx);
                        const tx1 = tx.getTx();
                        tx1.outputs.length.should.equal(1);
                        tx1.outputs[0][atomicsAccessor].should.equal(tx.amount);
                        done();
                    });
                });
            });
                        ...
                        server.createTx(txOpts, function (err, tx) {
                            ...
                            server.getPendingTxs({}, function (err, txs) {
                                ...
                                server.getBalance({}, function (err, balance) {
                                    should.not.exist(err);
                                    balance.totalAmount.should.equal(3.6e8);
                                    const amountInputs = lodash.sumBy(txs[0].inputs, atomicsAccessor);
            ...
            it('should accept a tx proposal signed with a custom key', function (done) {
                const reqPrivKey = new PrivateKey();
                ...
                const accessOpts = {
                    ...
                    signature: helpers.signRequestPubKey(serviceName, reqPubKey, xPrivKey),
                };
                ...
                server.addAccess(accessOpts, function (err) {
                    ...
                    helpers.stubUtxos(server, wallet, [1, 2], function () {
                        ...
                        server.createTx(txOpts, function (err, txp) {
                            ...
                            const publishOpts = {
                                txProposalId: txp.id,
                                proposalSignature: helpers.signMessage(serviceName, txp.getRawTx(), reqPrivKey),
                            };
            ...
            it('should be able to send max funds', function (done) {
                helpers.stubUtxos(server, wallet, [1, 2], function () {
                    ...
                    server.createTx(txOpts, function (err, tx) {
                        ...
                        t.toObject().outputs[0][atomicsAccessor].should.equal(tx.amount);
                        done();
                    });
                });
            });
            ...
            it('should shuffle outputs unless specified', function (done) {
                helpers.stubUtxos(server, wallet, 1, function () {
                    const txOpts = {
                        outputs: lodash.times(30, function (i) {
                            return {
                                toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                                amount: (i + 1) * 100e2,
                            };
                    ...
                    server.createTx(txOpts, function (err, txp) {
                        ...
                        should.exist(txp);
                        let t = txp.getTx();
                        let changeOutput = t.getChangeOutput()[atomicsAccessor];
                        let outputs = lodash.without(lodash.map(t.outputs, atomicsAccessor), changeOutput);

                        outputs.should.not.deep.equal(lodash.map(txOpts.outputs, 'amount'));
                        ...
                        server.createTx(txOpts, function (err, txp) {
                            ...
                            t = txp.getTx();
                            changeOutput = t.getChangeOutput()[atomicsAccessor];
                            outputs = lodash.without(lodash.map(t.outputs, atomicsAccessor), changeOutput);

                            outputs.should.deep.equal(lodash.map(txOpts.outputs, 'amount'));
                            done();
                        });
                    });
                });
        ...
        describe('Backoff time', function (done) {
            let server; let wallet; let txid; let clock;
            const _oldBackoffOffset = Defaults.BACKOFF_OFFSET;
            beforeEach(function (done) {
                Defaults.BACKOFF_OFFSET = 3;
                helpers.createAndJoinWallet(serviceName, 2, 2, function (s, w) {
                    server = s;
                    wallet = w;
                    helpers.stubUtxos(server, wallet, lodash.range(2, 6), function () {
                        done();
                    });
                });
            });
            ...
            it('should follow backoff time after consecutive rejections', function (done) {
                ...
                async.series([
                    ...
                    function (next) {
                        async.each(lodash.range(3), function (i, next) {
                            helpers.createAndPublishTx(server, txOpts, TestData.copayers[0].privKey_1H_0, function (tx) {
                                server.rejectTx({
                                    txProposalId: tx.id,
                                    reason: 'some reason',
        ...
        describe('UTXO Selection', function () {
            ...
            it('should use non-locked confirmed utxos when specified', function (done) {
                helpers.stubUtxos(server, wallet, [1.3, 'u2', 'u0.1', 1.2], function (utxos) {
                    ...
                    helpers.createAndPublishTx(server, txOpts, TestData.copayers[0].privKey_1H_0, function (tx) {
                        ...
                        server.getBalance({}, function (err, balance) {
                            should.not.exist(err);
                            balance.lockedConfirmedAmount.should.equal(helpers.toAtomic(serviceName, 2.5));
            ...
            it('should select a single utxo if within thresholds relative to tx amount', function (done) {
                helpers.stubUtxos(server, wallet, [1, '350bit', '100bit', '100bit', '100bit'], function () {
                    ...
                    server.createTx(txOpts, function (err, txp) {
                        ...
                        txp.inputs[0][atomicsAccessor].should.equal(35000);
            ...
            it('should return inputs in random order', function (done) {
                ...
                helpers.stubUtxos(server, wallet, lodash.range(1, 31), function (utxos) {
                    const txOpts = {
                        outputs: [{
                            toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                            amount: lodash.sumBy(utxos, atomicsAccessor) - 0.5e8,
                        }],
                        feePerKb: 100e2,
                    };
                    server.createTx(txOpts, function (err, txp) {
                        should.not.exist(err);
                        should.exist(txp);
                        const amounts = lodash.map(txp.inputs, atomicsAccessor);
                        amounts.length.should.equal(30);
                        lodash.every(amounts, function (amount, i) {
                            if (i == 0) {
                                return true;
                            }
            ...
            it('should select a confirmed utxos if within thresholds relative to tx amount', function (done) {
                helpers.stubUtxos(server, wallet, [1, 'u 350bit', '100bit', '100bit', '100bit'], function () {
                    ...
                    server.createTx(txOpts, function (err, txp) {
                        ...
                        txp.inputs[0][atomicsAccessor].should.equal(10000);
            ...
            it('should select smaller utxos if within fee constraints', function (done) {
                helpers.stubUtxos(server, wallet, [1, '800bit', '800bit', '800bit'], function () {
                    ...
                    server.createTx(txOpts, function (err, txp) {
                        ...
                        should.exist(txp);
                        txp.inputs.length.should.equal(3);
                        lodash.every(txp.inputs, function (input) {
                            return input == 100e2;
                        });
                        done();
                    });
            ...
            it('should select smallest big utxo if small utxos are insufficient', function (done) {
                helpers.stubUtxos(server, wallet, [3, 1, 2, '100bit', '100bit', '100bit'], function () {
                    ...
                    server.createTx(txOpts, function (err, txp) {
                        ...
                        should.exist(txp);
                        txp.inputs.length.should.equal(1);
                        txp.inputs[0][atomicsAccessor].should.equal(1e8);
                        done();
                    });
                });
            });
            ...
            it('should account for fee when selecting smallest big utxo', function (done) {
                const _old = Defaults.UTXO_SELECTION_MAX_SINGLE_UTXO_FACTOR;
            ...
            it('should select smallest big utxo if small utxos exceed maximum fee', function (done) {
                helpers.stubUtxos(server, wallet, [3, 1, 2].concat(lodash.times(20, function () {
                    return '1000bit';
                })), function () {
                    const txOpts = {
                        outputs: [{
            ...
            it('should select smallest big utxo if small utxos are below accepted ratio of txp amount', function (done) {
                helpers.stubUtxos(server, wallet, [9, 1, 1, 0.5, 0.2, 0.2, 0.2], function () {
                    ...
                    server.createTx(txOpts, function (err, txp) {
                        ...
                        should.exist(txp);
                        txp.inputs.length.should.equal(1);
                        txp.inputs[0][atomicsAccessor].should.equal(9e8);
                        done();
                    });
                });
            });
            ...
            it('should not fail with tx exceeded max size if there is at least 1 big input', function (done) {
                const _old1 = Defaults.UTXO_SELECTION_MIN_TX_AMOUNT_VS_UTXO_FACTOR;
                const _old2 = Defaults.MAX_TX_SIZE_IN_KB;
                Defaults.UTXO_SELECTION_MIN_TX_AMOUNT_VS_UTXO_FACTOR = 0.0001;
                Defaults.MAX_TX_SIZE_IN_KB = 2;

                helpers.stubUtxos(server, wallet, [100].concat(lodash.range(1, 20, 0)), function () {
                    const txOpts = {
                        outputs: [{
                            toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                            amount: 15e8,
                    ...
                    server.createTx(txOpts, function (err, txp) {
                        ...
                        should.exist(txp);
                        txp.inputs.length.should.equal(1);
                        txp.inputs[0][atomicsAccessor].should.equal(100e8);
            ...
            it('should prefer a higher fee (breaking all limits) if inputs have 6+ confirmations', function (done) {
                helpers.stubUtxos(server, wallet, ['2c 2000bit'].concat(lodash.times(20, function () {
                    return '100bit';
                })), function () {
                    const txOpts = {
                        outputs: [{
            ...
            it('should ignore utxos too small to pay for fee', function (done) {
                helpers.stubUtxos(server, wallet, ['1c200bit', '200bit'].concat(lodash.times(20, function () {
                    return '1bit';
                })), function () {
                    const txOpts = {
                        outputs: [{
            ...
            it('should use small utxos if fee is low', function (done) {
                helpers.stubUtxos(server, wallet, [].concat(lodash.times(10, function () {
                    return '30bit';
                })), function () {
                    const txOpts = {
                        outputs: [{
            ...
            it('should correct fee if resulting change would be below threshold', function (done) {
                helpers.stubUtxos(server, wallet, ['200bit', '500satoshi'], function () {
                    ...
                    server.createTx(txOpts, function (err, txp) {
                        should.not.exist(err);
                        txp.inputs.length.should.equal(1);
                        (lodash.sumBy(txp.inputs, atomicsAccessor) - txp.outputs[0].amount - txp.fee).should.equal(0);
                        const changeOutput = txp.getTx().getChangeOutput();
    ...
    describe('Transaction notes', function (done) {
        ...
        it('should include the note in tx history listing', function (done) {
            helpers.createAddresses(server, wallet, 1, 1, function (mainAddresses, changeAddress) {
                ...
                helpers.stubHistory(serviceName, txs);
                helpers.stubFeeLevels(serviceName, {
                    24: 10000,
                });
                server.editTxNote({
    ...
    describe('Single-address wallet', function () {
        let server; let wallet; let firstAddress;
        beforeEach(function (done) {
            helpers.createAndJoinWallet(serviceName, 1, 2, {
                singleAddress: true,
            }, function (s, w) {
                server = s;
                wallet = w;
        ...
        it('should not duplicate address on storage after TX creation', function (done) {
            helpers.stubUtxos(server, wallet, 2, function () {
                ...
                server.createTx(opts, function (err, txp) {
                    should.not.exist(err);
                    server.getStorage().fetchAddresses(wallet.id, function (err, addresses) {
                        should.not.exist(err);
                        addresses.length.should.equal(1);
                        done();
                    });
        ...
        it('should return inputs in random order', function (done) {
            ...
            helpers.stubUtxos(server, wallet, lodash.range(1, 31), function (utxos) {
                server.getSendMaxInfo({
                    ...
                    should.not.exist(err);
                    should.exist(info);
                    const amounts = lodash.map(info.inputs, atomicsAccessor);
                    amounts.length.should.equal(30);
                    lodash.every(amounts, function (amount, i) {
                        if (i == 0) {
                            return true;
    ...
    describe('#rejectTx', function () {
        let server; let wallet; let txid;

        beforeEach(function (done) {
            helpers.createAndJoinWallet(serviceName, 2, 2, function (s, w) {
                server = s;
                wallet = w;
                helpers.stubUtxos(server, wallet, lodash.range(1, 9), function () {
                    const txOpts = {
                        outputs: [{
                            toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                            amount: 10e8,
        ...
        describe('Multisig', function () {
            ...
            it('should fail on wrong number of invalid signatures', function (done) {
                server.getPendingTxs({}, function (err, txs) {
                    ...
                    const signatures = lodash.take(helpers.clientSign(tx, TestData.copayers[0].xPrivKey_44H_0H_0H), tx.inputs.length - 1);
                    server.signTx({
                        txProposalId: txid,
                        signatures: signatures,
                    }, function (err) {
            ...
            it('should fail to sign a non-pending TX', function (done) {
                async.waterfall([
                    ...
                    function (next) {
                        helpers.getAuthServer(serviceName, wallet.copayers[2].id, function (server) {
                            server.getTx({
                                txProposalId: txid
                            }, function (err, tx) {
                                should.not.exist(err);
    ...
    describe('#broadcastTx & #broadcastRawTx', function () {
        let server; let wallet; let txpid; let txid;
        beforeEach(function (done) {
            helpers.createAndJoinWallet(serviceName, 1, 1, function (s, w) {
                server = s;
                wallet = w;
        ...
        it('should broadcast a tx', function (done) {
            const clock = sinon.useFakeTimers(1234000, 'Date');
            helpers.stubBroadcast(serviceName);
            server.broadcastTx({
                txProposalId: txpid
            }, function (err) {
    ...
    describe('Tx proposal workflow', function () {
        ...
        it('other copayers should see pending proposal created by one copayer', function (done) {
            ...
            helpers.createAndPublishTx(server, txOpts, TestData.copayers[0].privKey_1H_0, function (txp) {
                should.exist(txp);
                helpers.getAuthServer(serviceName, wallet.copayers[1].id, function (server2, wallet) {
        ...
        it('tx proposals should not be finally accepted until quorum is reached', function (done) {
            ...
            async.waterfall([
                ...
                function (txp, next) {
                    helpers.getAuthServer(serviceName, wallet.copayers[1].id, function (server, wallet) {
    ...
    describe('#getTx', function () {
        let server; let wallet; let txpid;
        beforeEach(function (done) {
            helpers.createAndJoinWallet(serviceName, 2, 3, function (s, w) {
                server = s;
                wallet = w;
    ...
    describe('#getTxs', function () {
        ...
        beforeEach(function (done) {
            ...
            helpers.createAndJoinWallet(serviceName, 1, 1, function (s, w) {
                server = s;
                wallet = w;
                helpers.stubUtxos(server, wallet, lodash.range(1, 11), function () {
                    const txOpts = {
                        outputs: [{
                            toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                            amount: 0.1e8
                    ...
                    async.eachSeries(lodash.range(10), function (i, next) {
                        clock.tick(10 * 1000);
                        helpers.createAndPublishTx(server, txOpts, TestData.copayers[0].privKey_1H_0, function (txp) {
                            next();
                        });
        ...
        it('should pull 4 txs, down to to time 60', function (done) {
            server.getTxs({
                ...
                should.not.exist(err);
                const times = lodash.map(txps, 'createdOn');
    ...
    describe('#getNotifications', function () {
        ...
        beforeEach(function (done) {
            ...
            helpers.createAndJoinWallet(serviceName, 1, 1, function (s, w) {
                server = s;
                wallet = w;
                helpers.stubUtxos(server, wallet, lodash.range(4), function () {
                    const txOpts = {
                        outputs: [{
                            toAddress: '18PzpUFkFZE8zKWUPvfykkTxmB9oMR8qP7',
                            amount: 0.1e8
        ...
        it('should pull all notifications', function (done) {
            server.getNotifications({}, function (err, notifications) {
                should.not.exist(err);
                const types = lodash.map(notifications, 'type');
                types.should.deep.equal(['NewCopayer', 'NewAddress', 'NewAddress', 'NewTxProposal', 'NewTxProposal', 'NewTxProposal']);
                const walletIds = lodash.uniq(lodash.map(notifications, 'walletId'));
                walletIds.length.should.equal(1);
                walletIds[0].should.equal(wallet.id);
                const creators = lodash.uniq(lodash.compact(lodash.map(notifications, 'creatorId')));
        ...
        it('should pull notifications after a given notification id', function (done) {
            server.getNotifications({}, function (err, notifications) {
                should.not.exist(err);
                const from = lodash.head(lodash.takeRight(notifications, 2)).id; // second to last
                server.getNotifications({
                    notificationId: from,
                    minTs: +Date.now() - (60 * 1000),
                }, function (err, res) {
                    ...
                    res[0].id.should.equal(lodash.head(lodash.takeRight(notifications)).id);
                    done();
                });
            });
        });
        ...
        it('should return empty if no notifications found after a given id', function (done) {
            server.getNotifications({}, function (err, notifications) {
                should.not.exist(err);
                const from = lodash.head(lodash.takeRight(notifications)).id; // last one
                server.getNotifications({
                    notificationId: from,
                }, function (err, res) {
                    should.not.exist(err);
            ...
            helpers.getAuthServer(serviceName, wallet.copayers[1].id, function (server2) {
                ...
                server2.signTx({
                    ...
                    server.getPendingTxs({}, function (err, txs) {
                        ...
                        const clock = sinon.useFakeTimers(Date.now() + 1 + 24 * 3600 * 1000, 'Date');
                        server.removePendingTx({
                            txProposalId: txp.id
                        }, function (err) {
                            should.not.exist(err);
        ...
        it('should allow other copayer to remove a TX signed, after 24hrs', function (done) {
            helpers.getAuthServer(serviceName, wallet.copayers[1].id, function (server2) {
                ...
                server2.signTx({
                    ...
                    const clock = sinon.useFakeTimers(Date.now() + 2000 + Defaults.DELETE_LOCKTIME * 1000, 'Date');
                    server2.removePendingTx({
                        txProposalId: txp.id
                    }, function (err) {
                        should.not.exist(err);
    ...
    describe('#getTxHistory', function () {
        let server; let wallet; let mainAddresses; let changeAddresses;
        beforeEach(function (done) {
            blockchainExplorer.getBlockchainHeight = sinon.stub().callsArgWith(0, null, 1000);
            helpers.createAndJoinWallet(serviceName, 1, 1, function (s, w) {
                server = s;
        ...
        it('should get tx history from explorer', function (done) {
            helpers.stubHistory(serviceName, TestData.history);
            server.getTxHistory({}, function (err, txs) {
                should.not.exist(err);
                should.exist(txs);
                        ...
                        server.broadcastTx({
                            ...
                            const txs = [{
                                ...
                                inputs: [{
                                    ...
                                    amount: utxos[0][atomicsAccessor],
                                }],
                                outputs: [{
                                    address: changeAddresses[0].address,
                                    amount: 0.2e8 - 5460,
        ...
        it('should get various paginated tx history', function (done) {
            ...
            const txs = lodash.map(timestamps, function (ts, idx) {
                return {
                    txid: (idx + 1).toString(),
                    confirmations: ts / 10,
                    fees: 100,
            ...
            async.each(testCases, function (testCase, next) {
                server.getTxHistory(testCase.opts, function (err, txs) {
                    should.not.exist(err);
                    should.exist(txs);
                    lodash.map(txs, 'time').should.deep.equal(testCase.expected);
                    next();
                });
            }, done);
        });
        ...
        it('should handle invalid tx in  history ', function (done) {
            const h = lodash.clone(TestData.history);
            h.push({
                txid: 'xx'
            });
            helpers.stubHistory(serviceName, h);
            const l = TestData.history.length;
    ...
    describe('#getTxHistory cache', function () {
        let server; let wallet; let mainAddresses; let changeAddresses;
        const _threshold = Defaults.HISTORY_CACHE_ADDRESS_THRESOLD;
        beforeEach(function (done) {
        ...
        it('should store partial cache tx history from explorer', function (done) {
            ...
            const h = helpers.historyCacheTest(totalItems);
            helpers.stubHistory(serviceName, h);
            blockchainExplorer.getBlockchainHeight = sinon.stub().callsArgWith(0, null, 200);
            const storeTxHistoryCacheSpy = sinon.spy(server.getStorage(), 'storeTxHistoryCache');
            server.getTxHistory({
                skip: skip,
                limit: limit,
            }, function (err, txs) {
                ...
                const calls = storeTxHistoryCacheSpy.getCalls();
        ...
        it('should get real # of confirmations based on current block height', function (done) {
            const _confirmations = Defaults.CONFIRMATIONS_TO_START_CACHING;
            Defaults.CONFIRMATIONS_TO_START_CACHING = 6;
            Server._cachedBlockheight = null;

            const h = helpers.historyCacheTest(20);
            lodash.each(h, function (x, i) {
            ...
            server.getTxHistory({
                ...
                server.getTxHistory({
                    ...
                    lodash.head(txs).confirmations.should.equal(501);
                    lodash.last(txs).confirmations.should.equal(520);
                    ...
                    server._notify('NewBlock', {
                        ...
                        setTimeout(function () {
                            server.getTxHistory({
                                ...
                                should.not.exist(err);
                                lodash.head(txs).confirmations.should.equal(1001);
                                lodash.last(txs).confirmations.should.equal(1020);
        ...
        it('should get cached # of confirmations if current height unknown', function (done) {
            ...
            helpers.stubHistory(serviceName, h);
            const storeTxHistoryCacheSpy = sinon.spy(Storage.prototype, 'storeTxHistoryCache');
            ...
            server.getTxHistory({
                ...
                server.getTxHistory({
                    ...
                    should.not.exist(err);
                    txs.length.should.equal(20);
                    lodash.head(txs).confirmations.should.equal(500);
                    lodash.last(txs).confirmations.should.equal(519);
        ...
        it('should get returned # of confirmations for non cached txs', function (done) {
            ...
            server.getTxHistory({
                ...
                server.getTxHistory({
                    ...
                    should.not.exist(err);
                    txs.length.should.equal(20);
                    lodash.head(txs).confirmations.should.equal(0);
                    lodash.last(txs).confirmations.should.equal(19);
        ...
        describe('Downloading history', function () {
            ...
            beforeEach(function (done) {
                ...
                helpers.stubHistory(serviceName, h);
                server.getStorage().clearTxHistoryCache(server.walletId, function () {
                    done();
                });
            });

            it('from 0 to 200, two times, in order', function (done) {
                async.eachSeries(lodash.range(0, 200, 5), function (i, next) {
                    server.getTxHistory({
                        skip: i,
                        limit: 5,
                        ...
                        lodash.map(txs, 'txid').should.deep.equal(lodash.map(s, 'txid'));
            ...
            it('from 0 to 200, two times, random', function (done) {
                const indexes = lodash.range(0, 200, 5);
                async.eachSeries(lodash.shuffle(indexes), function (i, next) {
                    server.getTxHistory({
                        skip: i,
                        limit: 5,
                    }, function (err, txs, fromCache) {
                    ...
                    async.eachSeries(lodash.range(0, 190, 7), function (i, next) {
                        server.getTxHistory({
                            skip: i,
                            limit: 7,
                        }, function (err, txs, fromCache) {
                ...
                server.scan({}, function (err) {
                    ...
                    server.getWallet({}, function (err, wallet) {
                        ...
                        server.getStorage().fetchAddresses(wallet.id, function (err, addresses) {
                            ...
                            const paths = lodash.map(addresses, 'path');
                            lodash.difference(paths, expectedPaths).length.should.equal(0);
                            server.createAddress({}, function (err, address) {
                                should.not.exist(err);
                                address.path.should.equal('m/0/3');
                                done();
            ...
            it('should restore wallet balance', function (done) {
                async.waterfall([
                    ...
                    function (next) {
                        helpers.stubUtxos(server, wallet, [1, 2, 3], function (utxos) {
                            should.exist(utxos);
                            helpers.stubAddressActivity(serviceName, lodash.map(utxos, 'address'));
                            server.getBalance({}, function (err, balance) {
                                balance.totalAmount.should.equal(helpers.toAtomic(serviceName, 6));
                    ...
                    function (next) {
                        ...
                        helpers.createAndJoinWallet(serviceName, 1, 1, function (server, wallet) {
                            server.getBalance({}, function (err, balance) {
                                balance.totalAmount.should.equal(0);
                                next(null, server, wallet);
                            });
    ...
    describe('#startScan', function () {
        ...
        afterEach(function () {
            server.getMessageBroker().removeAllListeners();
        });
        ...
        it('should start an asynchronous scan', function (done) {
            ...
            server.getMessageBroker().onMessage(function (n) {
        ...
        it('should start multiple asynchronous scans for different wallets', function (done) {
            helpers.stubAddressActivity(serviceName, ['3K2VWMXheGZ4qG35DyGjA2dLeKfaSr534A']);
            ...
            new Server({
                ...
                server2.createWallet(opts, function (err, walletId) {
                    ...
                    server.joinWallet(copayerOpts, function (err, result) {
                        should.not.exist(err);
                        helpers.getAuthServer(serviceName, result.copayerId, function (server2) {
                            server.startScan({}, function (err) {
                                should.not.exist(err);
    ...
    describe('Push notifications', function () {
        ...
        it('should subscribe copayer to push notifications service', function (done) {
            helpers.getAuthServer(serviceName, wallet.copayers[0].id, function (server) {
                should.exist(server);
                server.pushNotificationsSubscribe({
                    ...
                    should.not.exist(err);
                    server.getStorage().fetchPushNotificationSubs(wallet.copayers[0].id, function (err, subs) {
                        should.not.exist(err);
                        should.exist(subs);
        ...
        it('should subscribe copayer to a tx confirmation', function (done) {
            helpers.getAuthServer(serviceName, wallet.copayers[0].id, function (server) {
                ...
                server.txConfirmationSubscribe({
                    ...
                    should.not.exist(err);
                    server.getStorage().fetchActiveTxConfirmationSubs(wallet.copayers[0].id, function (err, subs) {
                        should.not.exist(err);
                        should.exist(subs);
                        subs.length.should.equal(1);
                        const s = subs[0];
