...
var service = Networks.livenet.code;
...
if (config.cluster && !config.lockOpts.lockerServer) {
  throw service + ' Wallet Service: when running in cluster mode, locker server need to be configured';
}

if (config.cluster && !config.messageBrokerOpts.messageBrokerServer) {
  throw service + ' Wallet Service: when running in cluster mode, message broker server need to be configured';
}
...
function startInstance(cb) {
  ...
  expressApp.start(config, function(err) {
    if (err) {
      log.error(service + ' Wallet Service: Could not start instance', err);
      return;
    }
    ...
    log.info(service + ' Wallet Service: Running ' + instanceInfo);
    return;
  });
};
...
if (config.cluster && cluster.isMaster) {
  ...
  log.info(service + ' Wallet Service: Starting ' + instances + ' instances');
  ...
  cluster.on('exit', function(worker) {
    ...
    log.error(service + ' Wallet Service: Worker ' + worker.id + ' died');
    cluster.fork();
  });
  // Code to run if we're in a worker process
} else {
  log.info(service + ' Wallet Service: Listening on port: ' + port);
  startInstance();
};
